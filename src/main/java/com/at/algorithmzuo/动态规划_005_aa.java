...
public class 动态规划_005_aa {
    ...
    public static int[] topKSum1(int[] arr, int k) {
        ...
        f1(arr, 0, 0, allAns);
        allAns.sort((o1, o2) -> o1.compareTo(o2));
    ...
    public static void f1(int[] arr, int i, int sum, ArrayList<Integer> allAns) {
        if (i == arr.length) {
            allAns.add(sum);
        } else {
            // 要
            f1(arr, i + 1, sum + arr[i], allAns);
            // 不要
            f1(arr, i + 1, sum, allAns);
        }
    }
